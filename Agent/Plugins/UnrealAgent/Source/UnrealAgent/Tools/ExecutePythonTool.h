...
struct FExecutePythonTool : public FMcpTool
{
	...
	virtual FString ToolDescription() const override
	{
		return TEXT(
			...
			"- For a BATCH loop over MANY items (e.g. all actors/assets), wrap it in\n"
			"  `unreal.ScopedSlowTask` with a NON-EMPTY label and call\n"
			"  `enter_progress_frame(1)` every iteration so the editor stays responsive\n"
			"  and cancelable. Keep per-iteration work small.\n"
			"- Do NOT open a progress dialog (`make_dialog`) for introspection, a single\n"
			"  call, or a short loop — and never run a heavy synchronous load on the very\n"
			"  first frame inside the dialog. A dialog whose first frame blocks (or whose\n"
			"  body raises) freezes the editor at 0% with no way to cancel.\n"
			"- Do NOT brute-force unknown APIs by guessing class/property names in a loop.\n"
			"  If the API you need is not documented in the provided context, report the\n"
			"  limitation instead of probing repeatedly.\n"
			...
			"  visually verify the result, and `get_output_log` to check for any\n"
			"  warnings/errors the change produced.\n"
			"\n"
			"## Basic patterns\n"
			"```python\n"
			"# Editor subsystem (UE5 preferred — EditorLevelLibrary is deprecated)\n"
			...
			"\n"
			"# Find actor by name\n"
			"actor = next((a for a in actor_sub.get_all_level_actors()\n"
