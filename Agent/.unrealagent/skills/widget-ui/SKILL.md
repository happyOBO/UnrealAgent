...
- **그래프 로직은 도구로 끝까지 처리한다 — "수동 필요"로 미루지 말 것.** 아래는 모두
  ...
  - **배열 연산(와일드카드)**: `add_node CallFunction function=Array_Get`(또는 `Array_Length`/
    `Array_Add` 등)은 도구가 `UK2Node_CallArrayFunction`으로 만들어, 타입드 배열 출력
    (예: `GetInventoryItems`의 `OutItems: TArray<UItemInstance*>`)을 `connect_pins`로 연결하면
    와일드카드 원소 타입이 자동 해석된다. **배열 순회 때문에 게임 코드에 `GetXxxAt(int32)`
    인덱스 헬퍼를 추가하지 말 것** — `GetInventoryCount`로 루프 길이, `Array_Get(arr, i)`로 원소 접근.
    (`list_nodes`에서 핀이 'object'로 보여도 실제 그래프 타입은 정확하다 — 표시상의 문제일 뿐.)
